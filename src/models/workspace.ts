...
export const createWorkspaceService = async (data: { name: string; description?: string; styleColor?: string }) => {
  return await prisma.$transaction(async (tx) => {
    const createData: Prisma.WorkspaceCreateInput = {
      name: data.name,
      description: data.description,
      // @ts-ignore: Prisma client types might be stale in IDE, but tsc passes
